	...
	if(typeof UnitParameter.MEP !== 'undefined')
	{
		...
		if(numEP != 0)
		{
			...
			if(numEP.match(regex))
			{
				...
				if( mainasu == '-' )
				{
					text =  /* OT_EPFrameSetting.RecoveryString */val + space + OT_EPFrameSetting.Name;
				}
				else
				{
					text = /* OT_EPFrameSetting.UseString */val + space + OT_EPFrameSetting.Name;
				}
			}
		...
		if(numFP != 0)
		{
			...
			if(numFP.match(regex))
			{
				...
				else
				{
					if( OT_GetCheckOnlyFP(object) )
					{
						text = OT_FPFrameSetting.CheckString + val + OT_FPFrameSetting.Name;
					}
					else
					{
						text = /* OT_EPFrameSetting.UseString */val + space + OT_EPFrameSetting.Name; 
